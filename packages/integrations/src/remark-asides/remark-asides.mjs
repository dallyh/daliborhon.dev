	...
	return (tree, file) => {
		...
		visit(tree, (node) => {
			if (node.type === "containerDirective") {
				if (!callouts(locale)[node.name]) return;

				const callout = callouts(locale)[node.name];
				...
				if (!title) {
					title = callouts(locale)[node.name].title ?? node.name;
				}
 ...
 */
const callouts = (locale) => {
	return {
		note: {
			title: m.asides__note_title(null, { locale: locale }),
		...
		success: {
			title: m.asides__success_title(null, { locale: locale }),
		...
		caution: {
			title: m.asides__caution_title(null, { locale: locale }),
		...
		danger: {
			title: m.asides__danger_title(null, { locale: locale }),
		...
		tip: {
			title: m.asides__tip_title(null, { locale: locale }),
...
function extractLocaleFromFilePath(path) {
